...
class ProjectScreenshotController extends Controller
{
    ...
    public function store(Request $request, Project $project)
    {
        $validated = $request->validate([
            'screenshot' => 'required|image|mimes:png,jpg,webp|max:2048',
        ]);
        ...
        try {
            ...
            return redirect()->back()->with('error', 'Failed to added project screenshot'.$e->getMessage());
        }
    }
    ...
    public function destroy(ProjectScreenshot $projectScreenshot)
    {
        try {
            ...
            DB::rollBack();
            return redirect()->back()->with('error', 'Failed to delete project screenshot'.$e->getMessage());
        } 
    }
}
